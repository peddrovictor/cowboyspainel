...
import {
  CLASSES, CLASS_LABELS, CLASS_COLORS, CULTIVOS, DEFAULT_EVENT_TYPES,
...
function getEventTypes(data) {
  const types = toArr(data?.eventTypes || []);
  return types.length > 0 ? types : DEFAULT_EVENT_TYPES;
}
function getEventTypeNames(data) { return getEventTypes(data).map(t => t.name); }
function getEventWeight(data, typeName) {
  const t = getEventTypes(data).find(t => t.name === typeName);
  return t ? t.weight : 1;
}
function getEventWeightsMap(data) {
  return getEventTypes(data).reduce((a, t) => { a[t.name] = t.weight; return a; }, {});
}
...
function LoginScreen({ onLogin, onPlayerView }) {
  ...
  if (mode === "loading") return (
    <div className="login-wrap">
      <div style={{color:"var(--gold)",fontFamily:"'Oswald',sans-serif",letterSpacing:4}}>Carregando...</div>
    </div>
  );
...
function MembersTab({ data, save }) {
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Membros COWBOYS");
    XLSX.writeFile(wb, "COWBOYS_Membros.xlsx");
  };
  ...
  return (
    <div className="card">
      ...
      {showSync && (
        <div className="form-box">
          <div style={{marginBottom:8}}>
            <label style={{fontFamily:"'Oswald',sans-serif",fontSize:".6rem",letterSpacing:2,textTransform:"uppercase",color:"var(--gold)"}}>Sincronizar com Lista Atual do Clã</label>
            ...
            <div>
              ...
              {syncResult.newPlayers.length > 0 && (
                <div style={{marginBottom:12}}>
                  <div style={{fontFamily:"'Oswald',sans-serif",fontSize:".6rem",letterSpacing:2,textTransform:"uppercase",color:"var(--green-l)",marginBottom:6,paddingBottom:4,borderBottom:"1px solid rgba(56,121,74,.3)"}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill, minmax(200px, 1fr))",gap:4}}>
                    {syncResult.newPlayers.map((p, i) => (
                      <div key={i} style={{display:"flex",alignItems:"center",gap:6,padding:"5px 8px",background:"rgba(56,121,74,.08)",border:"1px solid rgba(56,121,74,.2)",borderRadius:3,fontSize:".82rem"}}>
                        ...
                        <span style={{fontSize:".6rem",color:cc(p.class),fontFamily:"'Oswald',sans-serif",letterSpacing:1}}>{p.class}</span>
              ...
              {syncResult.leftPlayers.length > 0 && (
                <div style={{marginBottom:12}}>
                  <div style={{fontFamily:"'Oswald',sans-serif",fontSize:".6rem",letterSpacing:2,textTransform:"uppercase",color:"var(--red-l)",marginBottom:6,paddingBottom:4,borderBottom:"1px solid rgba(155,44,44,.3)"}}>
      ...
      {showBulk && (
        <div className="form-box">
          <div style={{marginBottom:8}}>
            <label style={{fontFamily:"'Oswald',sans-serif",fontSize:".6rem",letterSpacing:2,textTransform:"uppercase",color:"var(--text-d)"}}>Colar Dados em Massa</label>
          ...
          {bulkPreview.length > 0 && (
            <div style={{marginBottom:10}}>
              <div style={{fontFamily:"'Oswald',sans-serif",fontSize:".6rem",letterSpacing:2,textTransform:"uppercase",color:"var(--gold)",marginBottom:6}}>
      ...
      {data.members.length===0
        ...
        : <>
          ...
          <div style={{display:"flex",alignItems:"center",gap:12,flexWrap:"wrap",marginBottom:14,padding:"10px 14px",background:"var(--bg)",borderRadius:4}}>
            <div style={{fontFamily:"'Oswald',sans-serif",fontSize:".7rem",letterSpacing:2,textTransform:"uppercase",color:"var(--gold)",marginRight:4}}>
            ...
            {CLASSES.map(c => classCounts[c] > 0 && (
              <div key={c} style={{display:"flex",alignItems:"center",gap:4}}>
                ...
                <span style={{fontFamily:"'Oswald',sans-serif",fontSize:".7rem",fontWeight:700,color:cc(c),letterSpacing:1}}>{c}</span>
...
function ClassChart({ members }) {
  ...
  return (
    <div style={{marginTop:16,background:"var(--bg-i)",border:"1px solid var(--border)",borderRadius:4,padding:16}}>
      <div style={{fontFamily:"'Oswald',sans-serif",fontSize:".75rem",fontWeight:700,color:"var(--gold)",letterSpacing:2,textTransform:"uppercase",marginBottom:12,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:8,marginTop:10,justifyContent:"center"}}>
        {CLASSES.map(c=>(
          <div key={c} style={{display:"flex",alignItems:"center",gap:4,fontSize:".7rem",color:counts[c]===0?"var(--text-d)":"var(--text)"}}>
            ...
            <span style={{fontFamily:"'Oswald',sans-serif",letterSpacing:1,fontWeight:600}}>{c}</span>
          ...
          {sorted.map(m => {
            ...
            return (
              ...
              >
                ...
                <span style={{fontSize:".6rem",color:cc(m.class),fontFamily:"'Oswald',sans-serif",letterSpacing:1,flexShrink:0}}>{m.class}</span>
              </div>
            );
          })}
        </div>
...
function EventsTab({ data, save }) {
  ...
  const saveEditEv = () => {
    ...
    const old = data.events.find(e=>e.id===editEvId);
    save({...data,events:data.events.map(e=>e.id===editEvId?{...e,name:editForm.name,type:editForm.type}:e)}, `Editou evento "${old?.name}" → "${editForm.name}" (${editForm.type})`);
    setEditEvId(null);
  };
  const toggleExpand = (id) => setExpanded(e=>({...e,[id]:!e[id]}));
  const sorted=[...(data.events||[])].sort((a,b)=>b.date.localeCompare(a.date));
  const months=["JAN","FEV","MAR","ABR","MAI","JUN","JUL","AGO","SET","OUT","NOV","DEZ"];
  const typeBadge={"TW":"b-red","World Boss":"b-gold","Draconica":"b-green","Marcial":"b-blue","default":"b-blue"};

  // Attendance ranking (weighted)
  ...
  const ranking = data.members.map(m => {
    ...
    const pts = (data.events||[]).filter(e => (e.present||[]).includes(m.id)).reduce((s,e) => s + (getEventWeight(data, e.type)||1), 0);
    return { ...m, attended, pts, pct: maxPossiblePts > 0 ? Math.round(pts/maxPossiblePts*100) : 0 };
  }).sort((a,b) => b.pts - a.pts || b.attended - a.attended);
        ...
        <div className="card" style={{marginBottom:14}}>
          <div className="card-t" style={{cursor:"pointer"}} onClick={()=>setShowRanking(!showRanking)}>
            ...
            <span style={{display:"flex",alignItems:"center",gap:8}}>
              ...
              <span style={{fontSize:".6rem",color:"var(--text-d)",fontFamily:"'Oswald',sans-serif",letterSpacing:1}}>{getEventTypes(data).map(t=>t.name+'×'+t.weight).join(' · ')}</span>
              {showRanking ? Ico.chevDown : Ico.chevRight}
            </span>
          </div>
          {showRanking && (
            <div className="tbl"><table>
              <thead><tr><th>#</th><th>Nome</th><th>Classe</th>{getEventTypes(data).map(t=><th key={t.id}>{t.name} (×{t.weight})</th>)}<th>Pts</th><th>%</th></tr></thead>
              <tbody>
                {ranking.map((m,i)=>{
                  const medalColor = i===0?"#FFD700":i===1?"#C0C0C0":i===2?"#CD7F32":null;
                  return (
                    <tr key={m.id}>
                      <td style={{fontFamily:"'Oswald',sans-serif",fontWeight:700,color:medalColor||"var(--text-d)",fontSize:medalColor?".95rem":".8rem"}}>
                        {i<3 ? ["🥇","🥈","🥉"][i] : i+1}
                      </td>
                      <td style={{fontWeight:600}}>{m.name}</td>
                      <td><span className="badge" style={{background:cc(m.class)+"22",color:cc(m.class),border:`1px solid ${cc(m.class)}55`}}>{m.class}</span></td>
                      {getEventTypes(data).map(t=>{
                        const c=getTypeCount(m,t.name), tot=getTypeTotal(t.name);
                        return <td key={t.id} style={{fontSize:".8rem"}}>{tot>0?<span style={{color:c>0?"var(--text)":"var(--text-d)"}}>{c}/{tot} <span style={{color:"var(--gold)",fontSize:".7rem"}}>({c*t.weight})</span></span>:"—"}</td>;
                      })}
                      <td style={{fontWeight:700,color:"var(--gold)",fontFamily:"'Oswald',sans-serif"}}>{m.pts}</td>
                      <td>
        ...
        {show&&(
          <div className="form-box">
            <div className="fr">
              ...
              <div className="fg"><label>Tipo</label><select value={form.type} onChange={e=>setForm({...form,type:e.target.value})}>{getEventTypeNames(data).map(t=><option key={t}>{t}</option>)}</select></div>
          ...
          :sorted.map(ev=>{
            ...
            return (
              <div key={ev.id} style={{marginBottom:8,background:"var(--bg-i)",border:"1px solid var(--border)",borderRadius:4,overflow:"hidden"}}>
                {/* Header */}
                {editEvId === ev.id ? (
                  <div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 14px",background:"rgba(201,168,76,.05)",flexWrap:"wrap"}}>
                    <input value={editForm.name} onChange={e=>setEditForm({...editForm,name:e.target.value})}
                      style={{flex:1,minWidth:150,padding:"4px 8px",fontSize:".85rem",fontWeight:600}} />
                    <select value={editForm.type} onChange={e=>setEditForm({...editForm,type:e.target.value})}
                      style={{padding:"4px 8px",fontSize:".8rem"}}>
                      {getEventTypeNames(data).map(t=><option key={t}>{t}</option>)}
                    </select>
                    <span style={{fontSize:".7rem",color:"var(--text-d)"}}>Peso: ×{getEventWeight(data, editForm.type)||1}</span>
                    <button className="btn btn-s btn-green" onClick={saveEditEv}>{Ico.check}</button>
                    <button className="btn btn-s btn-d" onClick={()=>setEditEvId(null)}>✗</button>
                  </div>
                ) : (
                  <div
      ...
      {data.members.length > 0 && (
        <div className="card" style={{marginBottom:14}}>
          <div style={{display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
            <div style={{fontFamily:"'Oswald',sans-serif",fontSize:".7rem",letterSpacing:2,textTransform:"uppercase",color:"var(--gold)",marginRight:8}}>
              <span style={{fontSize:"1.2rem",fontWeight:700,marginRight:4}}>{data.members.length}</span>membros
            </div>
            <div style={{width:1,height:24,background:"var(--border-g)"}}/>
            {CLASSES.map(c => classCounts[c] > 0 && (
                  ...
                  <div style={{padding:"0 16px 16px"}}>
                    ...
                    {confM.length > 0 && (
                      <div style={{display:"flex",gap:6,marginBottom:10,flexWrap:"wrap",alignItems:"center"}}>
                        <span style={{fontFamily:"'Oswald',sans-serif",fontSize:".55rem",letterSpacing:2,textTransform:"uppercase",color:"var(--text-d)"}}>Composição:</span>
                        {CLASSES.map(c => confClassCounts[c] > 0 && (
                          <span key={c} style={{fontSize:".6rem",padding:"2px 6px",borderRadius:2,background:cc(c)+"18",color:cc(c),border:`1px solid ${cc(c)}40`,fontFamily:"'Oswald',sans-serif",letterSpacing:1,fontWeight:600}}>
                    ...
                    {pending.length>0&&(
                      <div style={{marginBottom:12}}>
                        <div style={{fontFamily:"'Oswald',sans-serif",fontSize:".6rem",letterSpacing:2,textTransform:"uppercase",color:"var(--gold)",marginBottom:6,paddingBottom:4,borderBottom:"1px solid var(--border)"}}>Aguardando ({pending.length})</div>
                        {/* Sortable header */}
                        <div style={{display:"flex",alignItems:"center",gap:6,padding:"4px 8px",marginBottom:2}}>
                          <span onClick={()=>handleSort("name")} style={{flex:1,fontFamily:"'Oswald',sans-serif",fontSize:".55rem",letterSpacing:2,textTransform:"uppercase",color:"var(--text-d)",cursor:"pointer",userSelect:"none",display:"flex",alignItems:"center",gap:4}}>
                            Nome <SortIcon col="name"/>
                          </span>
                          <span onClick={()=>handleSort("class")} style={{fontFamily:"'Oswald',sans-serif",fontSize:".55rem",letterSpacing:2,textTransform:"uppercase",color:"var(--text-d)",cursor:"pointer",userSelect:"none",display:"flex",alignItems:"center",gap:4,marginRight:140}}>
                        ...
                        {sortMembers(pending).map(m=>(
                          <div key={m.id} style={{display:"flex",alignItems:"center",gap:6,padding:"5px 8px",marginBottom:3,fontSize:".85rem"}}>
                            ...
                            <span style={{fontSize:".65rem",color:cc(m.class),fontFamily:"'Oswald',sans-serif",letterSpacing:1,width:35,textAlign:"center"}}>{m.class}</span>
                    ...
                    <div className="tw-cols">
                      <div>
                        ...
                        {confM.length===0?<div style={{fontSize:".8rem",color:"var(--text-d)",fontStyle:"italic",padding:"4px 8px"}}>Ninguém confirmado</div>
                          :sortMembers(confM).map(m=>(<div key={m.id} className="tw-player go" onClick={()=>togglePlayer(week.id,m.id,"confirm")}><span style={{color:"var(--green-l)",marginRight:4}}>●</span><span style={{fontWeight:600,flex:1}}>{m.name}</span><span style={{fontSize:".7rem",color:cc(m.class),fontFamily:"'Oswald',sans-serif"}}>{m.class}</span></div>))}
                      </div>
                      <div>
      ...
      {weeks.length === 0 ? (
        ...
        <>
          <div style={{marginBottom:16}}>
            <label style={{fontFamily:"'Oswald',sans-serif",fontSize:".55rem",letterSpacing:2,textTransform:"uppercase",color:"var(--text-d)",display:"block",marginBottom:4}}>Selecionar Semana de TW</label>
            <select
              value={selectedWeek || ""}
                  ...
                  <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                    {filteredUnassigned.map(m => (
                      ...
                      >
                        ...
                        <span style={{fontSize:".65rem",color:cc(m.class),fontFamily:"'Oswald',sans-serif",letterSpacing:1}}>{m.class}</span>
                      </div>
                    ))}
                  </div>
                )}
                      ...
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10,gap:6}}>
                        <input
                          ...
                          style={{
                            ...
                            color:"var(--gold)",fontFamily:"'Oswald',sans-serif",fontSize:".8rem",fontWeight:700,
                            letterSpacing:2,padding:"2px 4px",flex:1,outline:"none",textTransform:"uppercase"
                          }}
                        />
                        <span style={{
                          fontFamily:"'Oswald',sans-serif",fontSize:".65rem",letterSpacing:1,
                        ...
                        <div style={{display:"flex",flexWrap:"wrap",gap:4,marginBottom:8}}>
                          {Object.entries(classCount).map(([cls, cnt]) => (
                            <span key={cls} style={{
                              ...
                              fontFamily:"'Oswald',sans-serif",letterSpacing:1,fontWeight:600
                            }}>
                              {cnt}x {cls}
                            </span>
                          ))}
                        ...
                        <div style={{display:"flex",flexDirection:"column",gap:3}}>
                          {ptPlayers.map((m, idx) => (
                            ...
                            >
                              <span style={{
                                width:16,height:16,borderRadius:2,display:"flex",alignItems:"center",justifyContent:"center",
                                background:"var(--border)",fontSize:".6rem",color:"var(--text-d)",fontFamily:"'Oswald',sans-serif",flexShrink:0
                              }}>
                                {idx + 1}
                              </span>
        ...
        <div className="card">
          ...
          {totalEvents === 0 ? (
            ...
            <>
              ...
              <div className="tbl"><table>
                <thead><tr><th>#</th><th>Nome</th>{getEventTypes(data).map(t=><th key={t.id}>{t.name} (×{t.weight})</th>)}<th>Pts</th><th>%</th></tr></thead>
                <tbody>
                  {filtered.map((m, i) => {
          ...
          {weeks.length === 0 ? <div className="empty">Nenhuma TW agendada no momento.</div>
            : (
              <div style={{textAlign:"center",padding:"30px 16px"}}>
                <div style={{fontFamily:"'Oswald',sans-serif",fontSize:".7rem",letterSpacing:3,textTransform:"uppercase",color:"var(--text-d)",marginBottom:8}}>Próxima TW</div>
                <div style={{fontFamily:"'Oswald',sans-serif",fontSize:"1.6rem",fontWeight:900,color:"var(--gold)",letterSpacing:3}}>{weeks[0].label}</div>
              </div>
            )
          }
        </div>
        ...
        <div style={{padding:"4px 0"}}>
          {RULES.map((r, i) => (
            <div key={i} style={{display:"flex",gap:10,padding:"6px 8px",fontSize:".85rem",alignItems:"flex-start"}}>
              <span style={{fontFamily:"'Oswald',sans-serif",fontSize:".7rem",fontWeight:700,color:"var(--gold)",flexShrink:0,marginTop:1,width:20,textAlign:"center"}}>{i+1}.</span>
        ...
        {queue.length === 0 ? (
          ...
          <div className="tbl"><table>
            ...
            <tbody>
              {queue.map((q, idx) => (
                <tr key={q.id}>
                  <td style={{fontFamily:"'Oswald',sans-serif",fontWeight:700,color:"var(--gold)",textAlign:"center",fontSize:"1rem"}}>
                    {idx + 1}º
                  </td>
      ...
      {filtered.length === 0 ? (
        ...
        days.map(day => (
          <div key={day} style={{marginBottom:16}}>
            <div style={{fontFamily:"'Oswald',sans-serif",fontSize:".6rem",letterSpacing:2,textTransform:"uppercase",color:"var(--gold-d)",marginBottom:6,paddingBottom:4,borderBottom:"1px solid var(--border)"}}>
              {dayLabel(day)}
            </div>
            {groups[day].map((l, i) => (
              <div key={i} style={{display:"flex",gap:10,padding:"6px 8px",fontSize:".82rem",borderBottom:"1px solid var(--border)",alignItems:"center"}}>
                ...
                <span style={{fontWeight:600,color:"var(--gold)",fontSize:".75rem",flexShrink:0,minWidth:60,fontFamily:"'Oswald',sans-serif",letterSpacing:1}}>
                  {l.user}
                </span>
...
/* ═══════════════ EVENT TYPES CONFIG ═══════════════ */
function EventTypesConfig({ data, save }) {
  ...
  const saveTypes = (newTypes, logMsg) => {
    save({ ...data, eventTypes: newTypes }, logMsg);
  };

  const addType = () => {
    if (!newName.trim()) return;
    if (types.some(t => t.name.toLowerCase() === newName.trim().toLowerCase())) return;
    const id = newName.trim().toLowerCase().replace(/\s+/g,"_");
    saveTypes([...types, { id, name: newName.trim(), weight: newWeight }], `Criou tipo de evento "${newName.trim()}" (peso ×${newWeight})`);
    setNewName(""); setNewWeight(1); setShowAdd(false);
  };

  const removeType = (id) => {
    const t = types.find(x=>x.id===id);
    saveTypes(types.filter(x=>x.id!==id), `Removeu tipo de evento "${t?.name}"`);
  };

  const startEdit = (t) => { setEditId(t.id); setEditForm({name:t.name,weight:t.weight}); };

  const saveEdit = () => {
    if (!editForm.name.trim()) return;
    const old = types.find(t=>t.id===editId);
    saveTypes(types.map(t=>t.id===editId?{...t,name:editForm.name.trim(),weight:editForm.weight}:t),
      `Editou evento "${old?.name}" → "${editForm.name.trim()}" (peso ×${editForm.weight})`);
    setEditId(null);
  };
  ...
  return (
    <div className="card" style={{marginTop:14}}>
      <div className="card-t">
        <span>Tipos de Evento & Pesos</span>
        <button className="btn" onClick={()=>setShowAdd(!showAdd)}>{Ico.plus} Novo Tipo</button>
      </div>

      {showAdd && (
        <div className="form-box">
          <div className="fr">
            <div className="fg" style={{flex:2}}><label>Nome do Evento</label><input value={newName} onChange={e=>setNewName(e.target.value)} placeholder="Ex: Nação, Corrida..."/></div>
          ...
          <div style={{display:"flex",gap:8}}>
            <button className="btn" onClick={addType} disabled={!newName.trim()}>{Ico.check} Criar</button>
            <button className="btn btn-d" onClick={()=>setShowAdd(false)}>Cancelar</button>
          </div>
        </div>
      )}
      ...
      <div className="tbl"><table>
        <thead><tr><th style={{width:40}}></th><th>Evento</th><th>Peso</th><th>Pontos por presença</th><th></th></tr></thead>
        <tbody>
          {types.map((t, idx) => (
            <tr key={t.id} style={editId===t.id?{background:"rgba(201,168,76,.05)"}:{}}>
              <td style={{whiteSpace:"nowrap"}}>
                <button className="btn btn-s" onClick={()=>moveType(t.id,-1)} disabled={idx===0} style={{opacity:idx===0?.3:1,marginRight:2}}>▲</button>
                <button className="btn btn-s" onClick={()=>moveType(t.id,1)} disabled={idx===types.length-1} style={{opacity:idx===types.length-1?.3:1}}>▼</button>
              </td>
              {editId===t.id ? (
                <>
                  <td><input value={editForm.name} onChange={e=>setEditForm({...editForm,name:e.target.value})} style={{padding:"4px 6px",fontSize:".85rem",width:"100%"}}/></td>
                  <td>
                    <select value={editForm.weight} onChange={e=>setEditForm({...editForm,weight:+e.target.value})} style={{padding:"4px 6px",fontSize:".85rem"}}>
                      {[1,2,3,4,5].map(n=><option key={n} value={n}>×{n}</option>)}
                    </select>
                  </td>
                  <td style={{color:"var(--gold)",fontFamily:"'Oswald',sans-serif",fontWeight:700}}>{editForm.weight} pts</td>
                  <td style={{whiteSpace:"nowrap"}}>
                    <button className="btn btn-s btn-green" onClick={saveEdit} style={{marginRight:4}}>{Ico.check}</button>
                    <button className="btn btn-s btn-d" onClick={()=>setEditId(null)}>✗</button>
                  </td>
                ...
                <>
                  <td style={{fontWeight:600,fontFamily:"'Oswald',sans-serif",letterSpacing:1}}>{t.name}</td>
                  <td><span className="badge b-gold">×{t.weight}</span></td>
                  <td style={{color:"var(--gold)",fontFamily:"'Oswald',sans-serif",fontWeight:700}}>{t.weight} pts</td>
                  <td style={{whiteSpace:"nowrap"}}>
                    <button className="btn btn-s" onClick={()=>startEdit(t)} style={{marginRight:4}}>{Ico.edit}</button>
                    <button className="btn btn-s btn-d" onClick={()=>removeType(t.id)}>{Ico.trash}</button>
                  </td>
                </>
              )}
            </tr>
      ...
      <div style={{marginTop:10,padding:"8px 12px",background:"var(--bg)",borderRadius:3,fontSize:".78rem",color:"var(--text-d)",lineHeight:1.6}}>
